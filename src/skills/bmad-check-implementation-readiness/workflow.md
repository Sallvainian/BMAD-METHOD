...
Load and read full config from {project-root}/_bmad/config.yaml (bmm section for module settings, root level for core) and {project-root}/_bmad/config.user.yaml for user preferences. If config is missing and bmad-method-setup skill is available, inform user to run setup first. Resolve:
